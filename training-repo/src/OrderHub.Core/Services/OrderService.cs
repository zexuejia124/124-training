...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var errors = await ApplyOrderItemsAsync(order, lines);
        if (errors.Count > 0)
    ...
    private async Task<IReadOnlyList<string>> ApplyOrderItemsAsync(Order order, IReadOnlyList<NewOrderLine> lines)
    {
